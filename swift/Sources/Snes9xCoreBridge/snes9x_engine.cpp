...
Snes9xEngine *activeEngine = nullptr;

void setError(Snes9xEngine *engine, const std::string &message);
bool saveBattery(Snes9xEngine *engine);
...
struct Snes9xEngine {
    ...
    Snes9xEngine() { audioBuffer.reserve(maxAudioFrames * audioChannelCount); }
};
...
Snes9xEngine *snes9x_engine_create(const char *systemDirectory)
{
    std::lock_guard<std::mutex> lock(claimMutex);
    if (engineClaimed) return nullptr;
    auto engine = std::make_unique<Snes9xEngine>();
...
void snes9x_engine_destroy(Snes9xEngine *engine)
{
    ...
    snes9x_engine_unload_rom(engine);
...
bool snes9x_engine_load_rom(Snes9xEngine *engine, const char *romPath, const char *batteryPath)
{
    if (engine == nullptr || engine != activeEngine || romPath == nullptr || romPath[0] == '\0') return false;
    if (engine->loaded) snes9x_engine_unload_rom(engine);
...
bool snes9x_engine_is_loaded(const Snes9xEngine *engine) { return engine != nullptr && engine->loaded; }
...
double snes9x_engine_frame_duration(const Snes9xEngine *engine) { return engine == nullptr ? 1.0 / 60.0988 : engine->frameDuration; }
const std::uint32_t *snes9x_engine_video_buffer(const Snes9xEngine *engine) { return engine == nullptr ? nullptr : engine->videoBuffer.data(); }
std::size_t snes9x_engine_video_pixel_count(const Snes9xEngine *engine) { return engine == nullptr ? 0 : engine->videoWidth * engine->videoHeight; }
std::uint32_t snes9x_engine_video_width(const Snes9xEngine *engine) { return engine == nullptr ? 0 : engine->videoWidth; }
std::uint32_t snes9x_engine_video_height(const Snes9xEngine *engine) { return engine == nullptr ? 0 : engine->videoHeight; }
const std::int16_t *snes9x_engine_audio_buffer(const Snes9xEngine *engine) { return engine == nullptr || engine->audioBuffer.empty() ? nullptr : engine->audioBuffer.data(); }
std::size_t snes9x_engine_audio_frame_count(const Snes9xEngine *engine) { return engine == nullptr ? 0 : engine->audioBuffer.size() / audioChannelCount; }
std::uint32_t snes9x_engine_audio_sample_rate(void) { return audioSampleRate; }
std::uint32_t snes9x_engine_audio_channel_count(void) { return audioChannelCount; }
...
void snes9x_engine_reset_inputs(Snes9xEngine *engine) { if (engine != nullptr) engine->inputMasks.fill(0); }
void snes9x_engine_reset(Snes9xEngine *engine, bool hardReset)
{
    if (engine == nullptr || !engine->loaded) return;
...
void snes9x_engine_clear_cheats(Snes9xEngine *engine) { if (engine != nullptr && engine->loaded) retro_cheat_reset(); }
const char *snes9x_engine_last_error(const Snes9xEngine *engine) { return engine == nullptr ? "The engine is unavailable or already in use." : engine->lastError.c_str(); }
