...
final class SNESGameController {
    private weak var engine: SNESEngine?
    private let connectionChanged: (Bool) -> Void
    private var observers: [NSObjectProtocol] = []
    private var boundControllers: [GCController] = []

    init(engine: SNESEngine, connectionChanged: @escaping (Bool) -> Void) {
        self.engine = engine
    ...
    private func releaseButtons(player: Int) {
        for button in SNESControllerButton.allCases {
    ...
    private func bindKeyboard() {
        GCKeyboard.coalesced?.keyboardInput?.keyChangedHandler = { [weak self] _, _, keyCode, pressed in
            let button: SNESControllerButton?
            switch keyCode {
