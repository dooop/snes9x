...
@Composable
internal fun Snes9xExternalControllerInput(
    engine: Snes9xEngine,
    modifier: Modifier = Modifier,
) {
    AndroidView(
        factory = { context -> Snes9xControllerInputView(context).also { it.engine = engine } },
...
private class Snes9xControllerInputView(
    context: Context,
) : View(context),
    InputManager.InputDeviceListener {
    var engine: Snes9xEngine? = null
    private val inputManager = context.getSystemService(InputManager::class.java)
    private val devicePlayers = mutableMapOf<Int, Int>()
    private val directions = mutableMapOf<Int, Set<Snes9xButton>>()
    ...
    override fun onGenericMotionEvent(event: MotionEvent): Boolean {
        ...
        val pressed =
            buildSet {
                if (horizontal < -AXIS_THRESHOLD) add(Snes9xButton.Left)
                if (horizontal > AXIS_THRESHOLD) add(Snes9xButton.Right)
                if (vertical < -AXIS_THRESHOLD) add(Snes9xButton.Up)
                if (vertical > AXIS_THRESHOLD) add(Snes9xButton.Down)
            }
    ...
    private fun handleKey(
        ...
        val button = keyCode.toSnes9xButton() ?: return false
    ...
    private fun releasePlayer(player: Int) {
        Snes9xButton.entries.forEach { button -> engine?.setButton(button, false, player) }
    }

    private companion object {
        const val AXIS_THRESHOLD = 0.5f
        val DIRECTION_BUTTONS = setOf(Snes9xButton.Up, Snes9xButton.Down, Snes9xButton.Left, Snes9xButton.Right)
        ...
        fun Int.toSnes9xButton(): Snes9xButton? =
            when (this) {
                ...
                KeyEvent.KEYCODE_BUTTON_A -> Snes9xButton.A
                KeyEvent.KEYCODE_BUTTON_B -> Snes9xButton.B
                KeyEvent.KEYCODE_BUTTON_X -> Snes9xButton.X
                KeyEvent.KEYCODE_BUTTON_Y -> Snes9xButton.Y
                KeyEvent.KEYCODE_BUTTON_L1 -> Snes9xButton.L
                KeyEvent.KEYCODE_BUTTON_R1 -> Snes9xButton.R
                KeyEvent.KEYCODE_BUTTON_START -> Snes9xButton.Start
                KeyEvent.KEYCODE_BUTTON_SELECT -> Snes9xButton.Select
                else -> null
            }
    }
}
